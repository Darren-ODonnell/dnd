...
const TeamsheetContainer = ({panel,team, subs, onDrop}) => {
    ...
    return (
        <Container className="teamsheet-container container mx-auto" style={{height:'800px'}}>
            <PanelContainer  panel  = {panel} source={source} onDrop = {onDrop}/>
            {/*<TeamContainer   team   = {team}  source={source} onDrop = {onDrop}/>*/}
...
const PanelContainer = ({panel, source, onDrop }) => {
    let nextRow = 0
...
//     // console.log("Panel: ActionContainer() - " + panel.map(m => {return m.name + " - "}))
//     return (
